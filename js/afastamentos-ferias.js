...
(function () {
  ...
  function calcularDias(raiz) {
    var di = RosterWork.data.paraData(raiz.querySelector('#fer-inicio').value);
    var df = RosterWork.data.paraData(raiz.querySelector('#fer-fim').value);
    if (!di || !df || df < di) return null;
    return Math.round((df - di) / 86400000) + 1;
  }

  function atualizarSalvar(raiz) {
    var btn = raiz.querySelector('#fer-salvar');
  ...
  function popularMilitares(raiz, unidadeId) {
    ...
    RosterWork.atestadosDados.buscarEfetivo().then(function (efetivo) {
      ...
      var tpl = document.getElementById('tpl-ferias-militar-opcao');
  ...
  function salvar(raiz) {
    ...
    var btnSalvar = raiz.querySelector('#fer-salvar'); if (btnSalvar) btnSalvar.disabled = true;   // evita duplo-envio (o véu já cobre; defesa a mais)
  ...
  /* abre o painel Nova férias (modo criação): clona o formulário e liga tudo */
  function abrirNovo() {
    ...
    var tplForm = document.getElementById('tpl-ferias-novo');
    var tplAcoes = document.getElementById('tpl-ferias-novo-acoes');
    var secoesForm = [];
    if (corpo && tplForm) {
    ...
    var inicio = raiz.querySelector('#fer-inicio');
    var fim = raiz.querySelector('#fer-fim');
    ...
    if (RosterWork.seletorUnidades) {
      var seletor = RosterWork.seletorUnidades.criar({
        modo: 'unico',
        dropdownEl: raiz.querySelector('#fer-unidade-dropdown'),
        arvoreEl: raiz.querySelector('#fer-unidade-arvore'),
        textoEl: raiz.querySelector('#fer-unidade-texto'),
        aoSelecionar: function (unidade) {
          var texto = raiz.querySelector('#fer-unidade-texto');
    ...
    var btnCancelar = raiz.querySelector('#fer-cancelar');
    if (btnCancelar) btnCancelar.addEventListener('click', tentarFechar);
    var btnSalvar = raiz.querySelector('#fer-salvar');
    if (btnSalvar) btnSalvar.addEventListener('click', function () { salvar(raiz); });
  }

  window.RosterWork.ferias = { abrirNovo: abrirNovo };
})();
