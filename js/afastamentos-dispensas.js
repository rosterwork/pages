/* ============================================================
   DISPENSAS — painel "Nova dispensa" (modo criação, só admin)
   Unidade (seletor em árvore) -> Militar, Tipo (gala/nojo/comum),
   Período (máscara + calendário) e Motivo. Indicador sugere fora/segue
   o fluxo pela regra dos 16 dias (o admin decide no toggle). Salva por
   RosterWork.afastamentosDados.inserirDispensa.
   ============================================================ */
(function () {
  'use strict';
  ...
  function aplicarDecisao(raiz) {
    ...
    var botoes = raiz.querySelectorAll('#dis-fora-toggle .aba');
  ...
  function atualizarFluxo(raiz) {
    ...
    if (box) {
      if (dias == null) {
        ...
        var titulo = raiz.querySelector('#dis-fluxo-titulo');
  ...
  function ligarTipo(raiz) {
    var gatilho = raiz.querySelector('#dis-tipo');
  ...
  function popularMilitares(raiz, unidadeId) {
    var gatilho = raiz.querySelector('#dis-militar');
    var menu = raiz.querySelector('#dis-militar-menu');
    ...
    RosterWork.afastamentosDados.buscarEfetivo().then(function (efetivo) {
      ...
      var tpl = document.getElementById('tpl-dispensa-militar-opcao');
  ...
  function salvar(raiz) {
    ...
    var corpo = {
      ...
      p_data_inicio: RosterWork.data.paraISO(raiz.querySelector('#dis-inicio').value),
      p_data_fim: RosterWork.data.paraISO(raiz.querySelector('#dis-fim').value),
    ...
    RosterWork.afastamentosDados.inserirDispensa(corpo).then(function (r) {
      ...
      if (r && r._falha === 'servidor') {
        ...
        RosterWork.avisar({ tipo: 'erro', mensagem: (r && r.error) || RosterWork.mensagens.afastamentos.falhaSalvarDispensa });
      }
    }).catch(function () {
      if (RosterWork.esconderVeuGlobal) RosterWork.esconderVeuGlobal();
      atualizarSalvar(raiz);
  ...
  function abrirNovo() {
    ...
    var tplForm = document.getElementById('tpl-dispensa-novo');
    var tplAcoes = document.getElementById('tpl-dispensa-novo-acoes');
    var secoesForm = [];
    if (corpo && tplForm) {
    ...
    var inicio = raiz.querySelector('#dis-inicio');
    var fim = raiz.querySelector('#dis-fim');
    ...
    var toggle = raiz.querySelector('#dis-fora-toggle');
    if (toggle) {
    ...
    if (RosterWork.seletorUnidades) {
      var seletor = RosterWork.seletorUnidades.criar({
        modo: 'unico',
        dropdownEl: raiz.querySelector('#dis-unidade-dropdown'),
        arvoreEl: raiz.querySelector('#dis-unidade-arvore'),
        textoEl: raiz.querySelector('#dis-unidade-texto'),
        aoSelecionar: function (unidade) {
          var texto = raiz.querySelector('#dis-unidade-texto');
    ...
    var btnCancelar = raiz.querySelector('#dis-cancelar');
    if (btnCancelar) btnCancelar.addEventListener('click', tentarFechar);
    var btnSalvar = raiz.querySelector('#dis-salvar');
    if (btnSalvar) btnSalvar.addEventListener('click', function () { salvar(raiz); });
  }

  window.RosterWork.dispensas = { abrirNovo: abrirNovo };
})();
